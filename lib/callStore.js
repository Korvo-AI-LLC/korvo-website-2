...
function shape(record) {
  ...
  const appointmentAt = record.appointmentAt || pick(record, ['appointment', 'appointment_time', 'appointmentAt', 'appointment_date', 'appointmentDate', 'appointment_datetime']);
  ...
  const reason     = data.reason     || pick(record, ['reason', 'reason_for_visit', 'reasonForVisit', 'chief_complaint', 'chiefComplaint']);
  const summary    = data.summary    || pick(record, ['summary', 'call_summary', 'callSummary']);
